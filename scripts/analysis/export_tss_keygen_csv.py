...
def main():
    if len(sys.argv) != 2:
        print("usage: export_tss_keygen_csv.py RUN_DIR", file=sys.stderr)
        sys.exit(2)

    run_dir = Path(sys.argv[1]).expanduser().resolve()
    summaries = sorted(run_dir.glob("artifacts/party*/keygen_summary_party_*.json"))

    if not summaries:
        summaries = sorted(run_dir.glob("party*/keygen_summary_party_*.json"))

    if not summaries:
        print(f"no keygen summary files found under {run_dir}", file=sys.stderr)
        sys.exit(1)
    ...
    keygen_ms_values = [int(r.get("keygen_ms", 0)) for r in rows if r.get("keygen_ms") is not None]
    keygen_time_ms = max(keygen_ms_values) if keygen_ms_values else ""
